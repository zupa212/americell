...
/**
 * Live device EMBEDDED in a black 9:16 Americell frame.
 *
 * The iframe loads `embedSrc` — the stream served through our own first-party
 * /embed proxy — so the device plays INSIDE our page (with our logo overlay)
 * instead of on the provider's domain. If the live socket can't connect in-frame
 * (upstream Origin check), the "Open full-screen" fallback runs it top-level via
 * the same-origin `/api/rentals/[id]/stream` redirect.
 */
export default function StreamViewer({
  ...
  // Fallback (top-level) URL keeps the provider's domain hidden via a redirect.
  const fallbackSrc = `/api/rentals/${rentalId}/stream`;
  const frameSrc = embedSrc ?? fallbackSrc;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Live device — EMBEDDED first-party, 9:16 black frame with our logo. */}
      ...
      {/* Fallback if the embedded live socket can't connect in-frame. */}
      <p className="text-center text-xs text-white/50">
        Stuck on “Connecting…”? Tap <span className="text-white/80">Reload</span>, or{" "}
        <a
          href={fallbackSrc}
          target="_blank"
