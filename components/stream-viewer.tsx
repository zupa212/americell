 ...
 *
 * The upstream live control runs over a WebSocket + a control popup that only
 * connect from a TOP-LEVEL page, so we open the live phone in a NEW WINDOW from
 * our branded frame, where it connects and plays. The stream URL is same-origin
 * (`/api/rentals/[id]/stream`, a server redirect resolves the upstream — and can
 * route through the first-party Cloudflare proxy when configured), so the
